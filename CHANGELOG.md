...
## [2.3.0] — 2026-06-26 — Novel adaptation mode for import

- **`/dm:dnd import` now adapts novels and prose fiction, not just game modules.** The extractor was always source-agnostic, but a novel is shaped differently from a module — continuous prose, a protagonist, no stat blocks, no keyed encounters. Import now detects prose-fiction sources and runs an **adaptation mode** that turns the book into a playable structured campaign: the player's character takes the **protagonist's seat** (the protagonist becomes the empty role the PC fills, never an NPC), the novel's plot turning points become enforced `key_beats` in `arc.md` (the same "consequence, not event" test the dynamic arc uses), and companions/antagonists carry over as NPCs. The storyline is preserved; the path to each beat is the player's, steered with world pressure rather than walls when they diverge from what the protagonist did.
- **Reflavored stat blocks with an out-of-character disclosure convention.** Novels carry no stat blocks, so every enemy runs on a mechanically appropriate SRD stat block chosen by **encounter math, not species**, narrated as its source identity. When such an encounter begins the DM states the true block once out of character — `(OOC: running this as an Adult Black Dragon — CR 14.)` — then narrates the fiction (the man in the grey coat). Attacks, movement, and abilities are reskinned freely; the numbers are rolled straight. Mappings are recorded in a **reflavor ledger** so a recurring villain keeps the same mechanics across sessions and a context compaction can't silently re-roll which block it "is." Optional `reflavor_ooc: off` suppresses the OOC line while still recording the mapping.
- **New `## Source Adaptation` anchor in `state.md`.** A compaction-resistant block (populated only for novel imports) holds the protagonist→PC substitution, the kept/recast/cut companion decisions, aspirational-powers notes, and the reflavor ledger. Read at every load alongside `## DM Style Notes` and `## Campaign Arc`.
- **Backward compatible — nothing to migrate.** Module imports, dynamic, and sandbox campaigns are unchanged; the `## Source Adaptation` section stays empty for them and no script behavior changed (the same `import_campaign.py` extraction handles both source kinds).
