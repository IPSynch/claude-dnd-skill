...
## [2.3.0] — 2026-06-27 — Novel adaptation + bestiary statblock import

- **Import a novel, not just a module.** `/dm:dnd import <source> --prose` runs the existing import pipeline with an adaptation analysis step tuned for prose fiction. A novel has no acts, encounter tables, or DCs and a single fixed protagonist the players won't be — so the importer no longer tries to *extract* a structure that isn't there; it **adapts** one: a campaign premise and party hook from the central conflict, a six-beat dynamic arc from the story's turning points (consequences, not scripted scenes), NPCs from characters (the protagonist becomes an NPC), locations from settings, encounters from conflicts, and factions from the story's sides. The result is a `type: dynamic` sandbox faithful to the source's world and characters, with the plot left for the party to drive. `import_campaign.py --info --prose` prints the adaptation checklist; the full procedure is in `SKILL-commands.md`.
- **Import bestiaries into the statblock library.** New `/dm:dnd bestiary import|list|remove` (`scripts/import_statblocks.py`) brings monster statblocks from a bestiary — Tome of Beasts, homebrew, any book you have the right to use — into the **supplemental** dataset, which `lookup.py` already merges on top of the bundled SRD without overwriting it. So an imported monster is immediately findable via `lookup.py monster "<name>"` and usable in combat. Two stages mirror campaign import: a script extracts and chunks the book (PyMuPDF column-aware, same as module import), Claude parses each statblock into a record, and the script validates and merges it — deriving `index` from the name, `xp` from CR via the standard 5e table, defaulting missing ability scores, stamping each record with its source book, de-duplicating against the SRD, and updating (not duplicating) on re-import. `--list` groups imported statblocks by book; `--remove`/`--remove-book` clean them out; `--ruleset 2024` targets the 2024 library.
- **Rights-aware by design.** Imported statblocks live in `data/dnd5e_supplemental.json` separate from the SRD. Open-licence bestiaries (e.g. Kobold Press's open Tome of Beasts content) are fine to keep in a shared library; closed/copyrighted books (the WotC *Monster Manual*) should stay in a local copy — the docs flag this at every entry point.
- **Backward compatible — nothing to migrate.** Module import is unchanged when `--prose` is absent; the statblock importer only adds to the supplemental file. Existing campaigns and datasets are untouched.
