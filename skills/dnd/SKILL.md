...
8. **Three more lazy reference layers — pull on demand, same discipline.** Beyond the chapter corpus: (a) **per-campaign supplements** at `supplements/` (extra module material scoped to this campaign — e.g. ToA's *Lost City of Mezro*, *Guide to ToA*) — read a supplement only when a scene calls for it; (b) the global **statblock library** (`monster-manual/`, ~3,000 creatures) — when a scene wants an encounter, pick a creature by type + CR from its `by-type/`/CR indexes and reflavor it to the setting rather than inventing numbers (see SKILL-scripts.md → *Statblock Library*); (c) global **DM rules** (`rules/`) — these *are* read in full at load and govern voice, rolling, and combat for every campaign, with a campaign's own `## DM Style Notes` taking precedence on conflict. Resolve all three with `paths.py` (`campaign-supplements <name>`, `monster-library`, `rules-dir`); each is optional and skipped silently if absent.
