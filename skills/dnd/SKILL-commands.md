...
5. **Mark this campaign active** (for the autosave hook): write `{"name": "<campaign-name>"}` to `$(python3 ${CLAUDE_SKILL_DIR}/scripts/paths.py runtime-dir)/active-campaign.json`. This is what `autosave_checkpoint.py` reads to know which campaign to checkpoint; a stale marker is harmless. Then read state.md, world.md, npcs.md (index only), and all characters/*.md
   ...
   - **Global DM rules (all campaigns) — read in full and obey for the whole session.** Resolve the directory with `python3 ${CLAUDE_SKILL_DIR}/scripts/paths.py rules-dir`. If it exists, read **every** `*.md` in it (e.g. `narration-style.md`, `house-rules.md`, `combat-rules.md`). These are the player's standing, cross-campaign instructions for prose voice, rolling/resource procedure, and combat conventions. **Precedence:** global rules are the baseline for *every* campaign; a campaign's own `state.md → ## DM Style Notes` overrides them where the two conflict. If the directory is absent, skip silently (older installs have no global rules — behavior is unchanged). A rules file may carry examples that name another campaign's PCs; apply its *general* procedure and ignore example-specific character references that don't fit the current party.
   - **Per-campaign supplements (imported campaigns) — lazy, like `source/`.** Resolve with `python3 ${CLAUDE_SKILL_DIR}/scripts/paths.py campaign-supplements <name>`. Do **NOT** load at session start. If the dir exists it holds extra source material scoped to this campaign (e.g. ToA's *Lost City of Mezro*, *Guide to ToA*); read a supplement on demand when a scene calls for it, the same way you pull a chapter's `source/<id>.md`. If absent, there is nothing to load.
   - **Statblock library (all campaigns) — register, do NOT read at load.** Resolve with `python3 ${CLAUDE_SKILL_DIR}/scripts/paths.py monster-library`. If present, just note it is available; pull statblocks on demand during play (see *Statblock Library* in SKILL-scripts.md). It is a large reference corpus — never load it at session start.
